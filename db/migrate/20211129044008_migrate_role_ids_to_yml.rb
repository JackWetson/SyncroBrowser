  ...
  def up
    ...
    say_with_time 'Migrating all existing MembershipRole records to the new role_ids column' do
      Membership.find_each do |membership|
        membership_role_ids = ActiveRecord::Base.connection.execute("SELECT roles.id FROM roles INNER JOIN membership_roles ON roles.id = membership_roles.role_id WHERE membership_roles.membership_id = #{membership.id}").to_a.map do |result|
          result['id']
        end
