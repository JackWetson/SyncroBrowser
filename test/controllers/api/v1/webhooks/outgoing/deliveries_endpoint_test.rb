...
module Api
  module V1
    module Webhooks
      module Outgoing
        class DeliveriesEndpointTest < Api::Test
          ...
          test 'index' do
            ...
            get "/api/v1/webhooks/outgoing/endpoints/#{@endpoint.id}/deliveries", params: { access_token: }
            assert_response :success

            # Make sure it's returning our resources.
            delivery_ids_returned = response.parsed_body['data'].map { |delivery| delivery.dig('attributes', 'id') }
          ...
          test 'show' do
            # Fetch and ensure nothing is seriously broken.
            get "/api/v1/webhooks/outgoing/deliveries/#{@delivery.id}", params: { access_token: }
            assert_response :success
          ...
          test 'create' do
            ...
            delivery_data = Api::V1::Webhooks::Outgoing::DeliverySerializer.new(build(:webhooks_outgoing_delivery, endpoint: nil)).serializable_hash.dig(
              :data, :attributes
            )
            delivery_data.except!(:id, :endpoint_id, :created_at, :updated_at)

            post "/api/v1/webhooks/outgoing/endpoints/#{@endpoint.id}/deliveries",
                 params: delivery_data.merge({ access_token: })
          ...
          test 'destroy' do
            ...
            delete "/api/v1/webhooks/outgoing/deliveries/#{@delivery.id}", params: { access_token: }
            assert_response :not_found
          end
        end
      end
