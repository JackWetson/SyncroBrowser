...
module Api
  module V1
    module Webhooks
      module Outgoing
        class DeliveryAttemptsEndpointTest < Api::Test
          ...
          test 'index' do
            ...
            get "/api/v1/webhooks/outgoing/deliveries/#{@delivery.id}/delivery_attempts", params: { access_token: }
            assert_response :success

            # Make sure it's returning our resources.
            delivery_attempt_ids_returned = response.parsed_body['data'].map do |delivery_attempt|
              delivery_attempt.dig('attributes', 'id')
            end
          ...
          test 'show' do
            # Fetch and ensure nothing is seriously broken.
            get "/api/v1/webhooks/outgoing/delivery_attempts/#{@delivery_attempt.id}", params: { access_token: }
            assert_response :success
            ...
            get "/api/v1/webhooks/outgoing/delivery_attempts/#{@delivery_attempt.id}",
          ...
          test 'create' do
            ...
            delivery_attempt_data = Api::V1::Webhooks::Outgoing::DeliveryAttemptSerializer.new(build(:webhooks_outgoing_delivery_attempt, delivery: nil)).serializable_hash.dig(
              :data, :attributes
            )
            ...
            post "/api/v1/webhooks/outgoing/deliveries/#{@delivery.id}/delivery_attempts",
                 params: delivery_attempt_data.merge({ access_token: })
          ...
          test 'destroy' do
            ...
            delete "/api/v1/webhooks/outgoing/delivery_attempts/#{@delivery_attempt.id}", params: { access_token: }
            assert_response :not_found
          end
        end
      end
