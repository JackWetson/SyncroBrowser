...
module Api
  module V1
    module Webhooks
      module Outgoing
        class EndpointsEndpointTest < Api::Test
          ...
          test 'index' do
            ...
            get "/api/v1/teams/#{@team.id}/webhooks/outgoing/endpoints", params: { access_token: }
            assert_response :success

            # Make sure it's returning our resources.
            endpoint_ids_returned = response.parsed_body['data'].map { |endpoint| endpoint.dig('attributes', 'id') }
          ...
          test 'show' do
            # Fetch and ensure nothing is seriously broken.
            get "/api/v1/webhooks/outgoing/endpoints/#{@endpoint.id}", params: { access_token: }
            assert_response :success
          ...
          test 'create' do
            ...
            endpoint_data = Api::V1::Webhooks::Outgoing::EndpointSerializer.new(build(:webhooks_outgoing_endpoint, team: nil)).serializable_hash.dig(
              :data, :attributes
            )
            endpoint_data.except!(:id, :team_id, :created_at, :updated_at)

            post "/api/v1/teams/#{@team.id}/webhooks/outgoing/endpoints",
                 params: endpoint_data.merge({ access_token: })
          ...
          test 'destroy' do
            ...
            assert_difference('Webhooks::Outgoing::Endpoint.count', -1) do
              delete "/api/v1/webhooks/outgoing/endpoints/#{@endpoint.id}", params: { access_token: }
              assert_response :success
            end
