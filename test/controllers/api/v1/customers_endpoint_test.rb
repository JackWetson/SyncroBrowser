...
class Api::V1::CustomersEndpointTest < Api::Test
  include Devise::Test::IntegrationHelpers
    ...
    test "index" do
      ...
      customer_ids_returned = response.parsed_body.dig("data").map { |customer| customer.dig("attributes", "id") }
    ...
    test "create" do
      ...
      customer_data = Api::V1::CustomerSerializer.new(build(:customer, team: nil)).serializable_hash.dig(:data, :attributes)
      customer_data.except!(:id, :team_id, :created_at, :updated_at)

      post "/api/v1/teams/#{@team.id}/customers",
        params: customer_data.merge({access_token: access_token})
