...
class InvitationDetailsTest < ApplicationSystemTestCase
  def within_membership_row(membership, &block)
    within "tr[data-id='#{membership.id}']", &block
  end

  def within_current_memberships_table(&block)
    within "tbody[data-model='Membership'][data-scope='current']", &block
  end

  def within_former_memberships_table(&block)
    within "tbody[data-model='Membership'][data-scope='tombstones']", &block
  end
  ...
  @@test_devices.each do |device_name, display_details|
    test "visitors can sign-up and manage team members with subscriptions #{subscriptions_enabled? ? 'enabled' : 'disabled'} on a #{device_name}" do
      ...
      path_for_new_invitation = %r{invitations/new}
      path_with_cancel_path_params = %r{invitations/new\?cancel_path=}
