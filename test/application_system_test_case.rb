...
class ApplicationSystemTestCase < ActionDispatch::SystemTestCase
  ...
  def within_team_menu_for(display_details, &block)
    within_primary_menu_for(display_details, &block)
  end
  ...
  def sign_in_from_homepage_for(_display_details)
  ...
  def sign_up_from_homepage_for(_display_details)
    # TODO: the tailwind port of bullet train doesn't currently support a homepage.
  ...
  def within_primary_menu_for(display_details, &block)
    open_mobile_menu if display_details[:mobile]
    within '.menu', &block
  end
  ...
  def assert_no_js_errors
    ...
    errors = errors.reject { |e| e.timestamp > last_timestamp } if last_timestamp.positive?
  ...
  def find_stimulus_controller_for_label(label, stimulus_controller, wrapper = false)
    if wrapper
      ...
      find('label', text: /\A#{label}\z/).first(:xpath,
                                                './/..').first("[data-controller=\"#{stimulus_controller}\"]")
    end
  end
